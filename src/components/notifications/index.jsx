...
const Notifications = () => {
    ...
    const getUserData = async () => {
      ...
      if (userDataDoc.exists()) {
        ...
        const companyQuery = query(collection(firestore, 'notifications'), where('company_id', '==', userData.company_id));
        const companyDocs = await getDocs(companyQuery);
        const notificationsData = []
        companyDocs.forEach((doc) => {
          const companyData = doc.data();
          notificationsData.push(companyData);
        });
            ...
            <main className="w-full h-screen flex self-center place-content-center place-items-center">
                <div className="w-96 text-gray-600 space-y-5 p-4 shadow-xl border rounded-xl">
                    <div className="text-center mb-6">
                        <h3 className="text-gray-800 text-xl font-semibold sm:text-2xl">Notifications</h3>
                    </div>
                        ...
                        <table className="w-full text-sm text-gray-600">
                            <thead>
                                <tr>
                                    ...
                                    <th className="px-4 py-2 text-gray-500 bg-transparent border">Message</th>
                                </tr>
                            </thead>
                            <tbody>
